...
class MaxStatsController(Controller):
    def setup_controls(self):
        ...
        self.max_power_rank_stats = files_cache.get("stats/max_power_rank.json", [])
        self.max_power_rank_stats.sort(key=lambda x: x["value"], reverse=True)
        self.max_light_stats = files_cache.get("stats/max_light.json", [])
    ...
    async def get_max_power_rank_interface(self):
        ...
        if max_power_rank_stats:
            ...
            mpri.controls.append(Text(loc("Max Power Rank data is unavailable in local mode.")))
    ...
    async def get_max_light_interface(self):
        ...
        if self.max_light_stats:
            ...
            self.mlbi.controls.append(Text(loc("Max Light data is unavailable in local mode.")))
